...
def parse_entries(xml_data):
    ...
    for entry in root.findall("atom:entry", ns):
        ...
        matched_terms = [term for term in PRIORITY_TERMS if term in relevance_text]
        score = min(5, max(1, 2 + len(matched_terms)))
        reasons = [
            "초록에서 사람의 움직임·자세·웨어러블/IMU 기반 평가와 연결되는 단서가 확인됩니다.",
            "재활승마 연구의 기승자 균형·자세 변화 측정 및 시각/바이오피드백 설계 후보로 검토할 수 있습니다.",
            "서비스디자인·참여디자인 관점에서는 치료사와 사용자가 이해 가능한 피드백 카드로 전환할 가치가 있습니다.",
        ]
        ...
        papers.append({
            ...
            "method": "strict arXiv abstract filter",
            "status": "Candidate",
            "keywords": SEARCH_KEYWORDS,
            "abstract": [summary],
            "relevance_score": score,
            "reasons": reasons,
            "pdf_deep_review_needed": score >= 4,
...
<body>
  ...
  <p>
    ...
    <span class=\"badge\">점수 {esc(paper.get("relevance_score", ""))}/5</span>
    <span class=\"badge\">PDF deep review: {"필요" if paper.get("pdf_deep_review_needed") else "선택"}</span>
  </p>

  <section>
    <h2>Candidate Card</h2>
    <p><strong>제목:</strong> {esc(paper["title"])}</p>
    <p><strong>저자:</strong> {esc(paper["authors"])}</p>
    <p><strong>링크:</strong> <a href=\"{esc(paper["link"])}\">{esc(paper["link"])}</a></p>
    <p><strong>읽을 가치 점수:</strong> {esc(paper.get("relevance_score", ""))}/5</p>
    <div class=\"box\">
      <ul>
        {"".join(f"<li>{esc(item)}</li>" for item in paper.get("reasons", []))}
      </ul>
    </div>
  </section>
</body>
...
def report_page(papers):
    if papers:
        paper_list = "\n".join(
            f"""
            <article class="paper">
              <p><span class="badge">arXiv: {esc(p["arxiv_id"])}</span><span class="badge">{esc(p["published"])}</span><span class="badge">점수 {esc(p.get("relevance_score", ""))}/5</span></p>
              <h3><a href="../{esc(p["page"])}">{esc(p["title"])}</a></h3>
              <ul>{"".join(f"<li>{esc(item)}</li>" for item in p.get("reasons", []))}</ul>
              <p><strong>PDF deep review:</strong> {"필요" if p.get("pdf_deep_review_needed") else "선택"}</p>
